<template>
    <app-layout>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tr v-for="(post, key) in posts.data" :key="post.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <inertia-link :href="route('posts.edit', post)" class="text-indigo-600 hover:text-indigo-900 mr-5">Edit</inertia-link>
                                            <a href="#" class="text-red-600 hover:text-red-900" @click="postToDeleteIndex = key">Delete</a>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
        ...
        <dialog-modal :show="postToDeleteIndex !== null" @close="postToDeleteIndex = null">
            <template #title>
                Delete User "{{posts.data[postToDeleteIndex].name}}"
            </template>
            <template #content>
                If you deleted the User, All related data will be deleted as well and you won't be able to restore it again.
            </template>
            <template #footer>
                <secondary-button @click="postToDeleteIndex = null" class="mx-2">
                    Cancel
                </secondary-button>
                <danger-button @click="deleteUser(posts.data[postToDeleteIndex].id)">
                    DELETE!
                </danger-button>
            </template>
        </dialog-modal>
